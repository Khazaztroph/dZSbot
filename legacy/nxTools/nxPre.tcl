...
proc ::nxTools::Pre::GetVolumeFreeSpace {volName} {
    # Inside ioFTPD, query through its native disk API. This stays in the
    # service process and therefore uses netmounts.cfg credentials for UNC.
    if {[llength [info commands ::ioDisk]]} {
        set diskInfo [::ioDisk info $volName]
        set freeBytes [string trim [lindex $diskInfo 0]]
        if {![string is wideinteger -strict $freeBytes] || $freeBytes < 0} {
            error "invalid ioDisk result for '$volName': '$diskInfo'"
        }
        return $freeBytes
    }

    # TWAPI 2.0b11 cannot be loaded by Tcl 9. DriveInfo only accepts local
    # drive roots, so use the Windows API as a standalone-test fallback.
    set volName [string map {/ \\} [string trim $volName]]
    if {![string match {*\\} $volName]} {append volName \\}
    set script {
        param([string]$drive)
...
public static class NxDiskSpace {
...
'@
        ...
        [UInt64]$total = 0
        [UInt64]$free = 0
        if (![NxDiskSpace]::GetDiskFreeSpaceEx($drive, [ref]$available,
                [ref]$total, [ref]$free)) {
    ...
    set freeBytes [string trim [exec -- powershell.exe -NoLogo -NoProfile \
        -NonInteractive -Command "& {$script}" -drive $volName]]
    if {![string is wideinteger -strict $freeBytes] || $freeBytes < 0} {
        error "invalid free-space result for '$volName': '$freeBytes'"
    }
    return $freeBytes
}
...
proc ::nxTools::Pre::Release {argList} {
    ...
    # If the pre destination is on a different drive or UNC share, ensure that there
    # is adequate space for the release (and a 10MB "safety" buffer).
    # ioFTPD 8.1 netmounts.cfg supplies Session 0 credentials for UNC targets.
    ...
    set destDrive [lindex [file split $destRealPath] 0]
       ...
       if {![string equal -nocase $sourceDrive $destDrive]} {
	       	if {[catch {GetVolumeFreeSpace [file dirname $destRealPath]} freeBytes]} {
               ErrorLog PreCheckSpace $freeBytes
               LinePuts "Unable to check available space on target drive, contact a siteop."
               return 1
           }
           set freeBytes [string trim $freeBytes]
           if {![string is wideinteger -strict $freeBytes] || $freeBytes < 0} {
               ErrorLog PreCheckSpace "invalid free-space result for $destDrive: '$freeBytes'"
               LinePuts "Unable to check available space on target drive, contact a siteop."
               return 1
           }
           set checkSize [expr {wide($totalSize) + (10*1024)}]
           set volume(free) [expr {wide($freeBytes) / 1024}]
