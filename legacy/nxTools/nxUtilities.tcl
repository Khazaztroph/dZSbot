...
proc ::nxTools::Utils::GetVolumeInfo {volName} {
    # TWAPI 2.0b11 is built for Tcl 8.x and cannot be loaded by Tcl 9.
    # Query the same Win32_LogicalDisk fields through Windows PowerShell.
    set script {
        param([string]$drive)
        $disk = [System.IO.DriveInfo]::new($drive)
        [Console]::Out.Write([string]::Join("`t", @(
            [string]$disk.FreeSpace,
            [string]$disk.Size,
    ...
    set output [exec -- powershell.exe -NoLogo -NoProfile -NonInteractive \
        -Command "& {$script}" -drive $volName]
    if {[llength [set fields [split $output "\t"]]] != 4} {
        error "unexpected volume information for $volName"
    }
    return $fields
}
...
proc ::nxTools::Utils::SiteDrives {} {
    ...
    foreach volName [file volumes] {
        ...
        if {[catch {GetVolumeInfo $volName} volInfo]} {
        	continue
        }
            lassign $volInfo volume(free) volume(total) volume(fs) volume(name)
