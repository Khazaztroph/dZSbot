...
# Tcl 9-compatible replacements for the SQLite functions formerly supplied
# by the legacy nxHelper/TclSQLite build.
proc ::nxLib::DbStrCaseEq {left right} {
    return [string equal -nocase $left $right]
}

proc ::nxLib::DbStrCaseEqN {left right length} {
    return [string equal -nocase -length $length $left $right]
}
...
proc ::nxLib::DbOpenFile {dbProc fileName} {
    ...
    $dbProc function StrCaseEq -argcount 2 ::nxLib::DbStrCaseEq
    $dbProc function StrCaseEqN -argcount 3 ::nxLib::DbStrCaseEqN
    return
}
